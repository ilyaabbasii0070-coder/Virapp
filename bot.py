...
def crypto_payment_text(total_toman: int, trx_amount: float | None) -> str:
    if trx_amount is not None:
        trx_line = f"💎 <b>معادل ترون (TRX):</b>  <code>{trx_amount}</code>  TRX"
    else:
        trx_line = "⏳ <b>در حال دریافت قیمت لحظه‌ای...</b>"
    return (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "📋 <b>مراحل پرداخت:</b>\n\n"
        "   1️⃣  مقدار دقیق TRX بالا را به آدرس فوق ارسال کنید\n"
        "   2️⃣  پس از واریز، دکمه ✅ <b>واریز شد</b> را بزنید\n"
        "   3️⃣  عکس رسید یا اسکرین‌شات تراکنش را ارسال کنید\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "⚠️ <b>نکات مهم:</b>\n\n"
        "   🔹 فقط از شبکه <b>TRON (TRC-20)</b> استفاده کنید\n"
        "   🔹 مقدار TRX را دقیقاً طبق نرخ لحظه ارسال کنید\n"
        "   🔹 آدرس ولت را با یک ضربه کپی کنید\n"
        "   🔹 بعد از واریز روی دکمه «واریز شد» بزنید\n"
        "   🔹 زمان بررسی: کمتر از ۳۰ دقیقه\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🕐 <b>آخرین بروزرسانی قیمت:</b>  {datetime.now().strftime('%H:%M:%S')}"
    )
...
OFFLINE_MSG = (
    "🔧 <b>ربات موقتاً در دسترس نیست</b> 🛠️\n\n"
...
def cb_bot_off(call):
    ...
    bot.send_message(call.message.chat.id,
        ...
        "برای روشن کردن، دکمه «🟢 روشن کردن ربات» را بزنید."
    )
...
def cb_bot_on(call):
    ...
    bot.send_message(call.message.chat.id,
        "🟢 <b>ربات روشن شد!</b>\n\n"
        "ربات مجدداً فعال است و کاربران می‌توانند خریدشان را انجام دهند."
    )
...
def _ask_payment(chat_id, user_id):
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("🔷 پرداخت با ترون (TRX)", callback_data="pay_crypto"),
    )
    bot.send_message(
        chat_id,
        f"💳 <b>مرحله پرداخت</b> 🧾\n\n"
...
def _start_crypto_payment(user_id, chat_id, state):
    bot.send_message(chat_id, "⏳ <b>در حال دریافت قیمت لحظه‌ای ترون...</b>")
    total   = state["total_price"]
    ...
    set_state(user_id, step="shop_crypto_wait",
              crypto_msg_id=sent.message_id,
              crypto_chat_id=chat_id,
...
def cb_crypto_paid(call):
    ...
    bot.send_message(
        call.message.chat.id,
        f"📸 <b>ارسال رسید پرداخت کریپتو</b> 🔷\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"✅ خوشحالیم که واریز انجام دادید!\n\n"
        f"💰 <b>مبلغ واریزی:</b>  {fmt(total)} تومان\n"
        f"🔷 <b>معادل TRX:</b>  {trx_str}\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "📌 <b>مرحله آخر:</b>\n\n"
        "   🖼️  لطفاً یک <b>اسکرین‌شات</b> از تراکنش انجام‌شده\n"
        "       یا <b>رسید پرداخت</b> خود را همین‌جا ارسال کنید\n\n"
        "   📋  رسید باید شامل موارد زیر باشد:\n"
        "       🔹 مقدار TRX ارسال‌شده\n"
        "       🔹 آدرس مقصد (ولت ما)\n"
        "       🔹 تاریخ و زمان تراکنش\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "⏳ <b>پس از ارسال رسید، سفارش شما بررسی می‌شود.</b>\n"
        "🕐 زمان بررسی: کمتر از ۳۰ دقیقه\n\n"
        f"❓ سوال دارید؟ @{SUPPORT_USERNAME}\n\n"
...
def _create_order_and_notify(user_id, chat_id, state, payment_method):
    ...
    if payment_method == "wallet":
        ...
        bot.send_message(chat_id,
            ...
            "📌 <b>مراحل پرداخت:</b>\n"
            f"  1️⃣  مبلغ <b>{fmt(total)} تومان</b> را واریز کنید\n"
            "  2️⃣  تصویر رسید بانکی را ذخیره کنید\n"
            "  3️⃣  رسید را همین‌جا در چت ارسال کنید\n\n"
            "⏳ زمان بررسی: کمتر از ۳۰ دقیقه\n\n"
            "👇 <b>تصویر رسید واریزی خود را ارسال کنید:</b>"
        )
...
def handle_all_photos(msg):
    ...
    elif step == "shop_crypto_receipt":
        _handle_crypto_receipt(msg)
...
def _handle_shop_receipt(msg):
    ...
    plan    = PLANS.get(order["plan_key"], {"label": "---", "gb": "?", "days": "?"})
...
def _handle_crypto_receipt(msg):
    ...
    names    = state.get("names", [])
    plan     = PLANS.get(plan_key, {"label": "---", "gb": "?", "days": "?"})
    names_t  = "\n".join([f"  {i+1}. {n}" for i, n in enumerate(names)])
    trx_amt  = toman_to_trx(total)
    trx_str  = f"{trx_amt} TRX" if trx_amt else "---"
    ...
    with get_db() as conn:
        ...
        for name in names:
            conn.execute(
                "INSERT INTO order_services(order_id,user_id,service_name,plan_key) VALUES(?,?,?,?)",
                (order_id, msg.from_user.id, name, plan_key)
            )
        conn.commit()
    ...
    clear_state(msg.from_user.id)
    bot.send_message(msg.chat.id,
        "📥 <b>رسید پرداخت کریپتو شما دریافت شد!</b> ✅\n\n"
        "🔷 رسید TRX شما به تیم پشتیبانی ارسال شد.\n\n"
...
def adm_receive_config(msg):
    if msg.text and msg.text.strip() == "/done":
        ...
        subs     = state.get("subs", [])
        if not configs:
            return bot.send_message(msg.chat.id, "⚠️ هیچ کانفیگی ثبت نشده.")
    ...
    if not text:
        return bot.send_message(msg.chat.id, "⚠️ متن خالی است.")
...
def _deliver_configs(order_id, configs, subs):
    ...
    for i, svc in enumerate(svc_rows):
        ...
        full_text = (
            ...
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "🔐 <b>کانفیگ اتصال</b>\n"
            "👆 <i>روی آن بزنید تا کپی شود</i>\n\n"
            f"<code>{safe_cfg}</code>\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        )
        if sub:
            safe_sub  = html_lib.escape(sub)
            full_text += (
                "🔗 <b>ساب‌لینک</b>\n"
                "👆 <i>روی آن بزنید تا کپی شود — یا دکمه زیر را بزنید</i>\n\n"
                f"<code>{safe_sub}</code>\n\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            )
        full_text += (
            "📌 <b>راهنمای استفاده:</b>\n\n"
            "  1️⃣  کانفیگ بالا را کپی کرده در اپ ایمپورت کنید\n"
            "  2️⃣  یا دکمه «اتصال با ساب‌لینک» را بزنید\n"
            "  3️⃣  یا QR کد بالایی را با اپ اسکن کنید\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"🆘 <b>پشتیبانی:</b> @{SUPPORT_USERNAME}\n"
            "💙 <b>از اعتماد شما سپاسگزاریم!</b> 🌟"
        )
...
def rename_service(msg):
    ...
    if not svc_id:
        return clear_state(msg.from_user.id)
...
def _show_my_services(chat_id, user_id):
    with get_db() as conn:
        all_svcs = conn.execute("SELECT * FROM order_services WHERE config_text IS NOT NULL ORDER BY id DESC").fetchall()
...
def wallet_amount(msg):
    ...
    bot.send_message(msg.chat.id,
        ...
        f"  💳 شماره کارت:\n  <code>{CARD_NUMBER}</code>\n\n"
        f"  👤 به نام: <b>{CARD_OWNER}</b>\n\n"
        f"  1️⃣  مبلغ <b>{fmt(amount)} تومان</b> را واریز کنید\n"
        "  2️⃣  تصویر رسید را در همین چت ارسال کنید\n\n"
...
def cb_ap_change_wallet(call):
    bot.answer_callback_query(call.id)
    set_state(ADMIN_ID, step="adm_change_wallet")
    bot.send_message(call.message.chat.id,
        f"🔷 <b>تغییر آدرس ولت TRX</b>\n\n"
        f"آدرس فعلی: <code>{TRX_WALLET}</code>\n\n"
...
def adm_change_card(msg):
    ...
    if len(new_card) < 10 or not new_card.isdigit():
        return bot.send_message(msg.chat.id, "⚠️ شماره کارت نامعتبر است (فقط اعداد، حداقل ۱۰ رقم).")
    CARD_NUMBER = new_card
    save_setting("card_number", CARD_NUMBER)
    clear_state(ADMIN_ID)
...
def adm_change_owner(msg):
    ...
    if len(new_owner) < 3:
        return bot.send_message(msg.chat.id, "⚠️ نام باید حداقل ۳ کاراکتر باشد.")
    CARD_OWNER = new_owner
    save_setting("card_owner", CARD_OWNER)
    clear_state(ADMIN_ID)
    bot.send_message(msg.chat.id, f"✅ نام صاحب کارت تغییر یافت!\n\nنام جدید: <b>{CARD_OWNER}</b>")
...
def adm_change_wallet(msg):
    global TRX_WALLET
    new_wallet = (msg.text or "").strip()
    if len(new_wallet) < 10:
        return bot.send_message(msg.chat.id, "⚠️ آدرس ولت نامعتبر است.")
    TRX_WALLET = new_wallet
    save_setting("trx_wallet", TRX_WALLET)
    clear_state(ADMIN_ID)
    bot.send_message(msg.chat.id, f"✅ آدرس ولت TRX تغییر یافت!\n\nآدرس جدید: <code>{TRX_WALLET}</code>")
...
def adm_add_product_price(msg):
    try:
        ...
        return bot.send_message(msg.chat.id, "⚠️ قیمت معتبر وارد کنید.")
    state = get_state(ADMIN_ID)
    gb, days = state["new_gb"], state["new_days"]
    icons    = {1: "⚡", 2: "🚀", 3: "🔥", 5: "💥"}
    icon     = icons.get(gb, "📦")
    ...
    with get_db() as conn:
        conn.execute("INSERT INTO products(plan_key,label,gb,days,price) VALUES(?,?,?,?,?)", (plan_key, label, gb, days, price))
        conn.commit()
    reload_plans()
    clear_state(ADMIN_ID)
    bot.send_message(msg.chat.id,
...
def _show_user_detail(chat_id, uid):
    ...
    bot.send_message(chat_id,
        ...
        f"👤 نام: {u['full_name'] or '---'}\n"
        f"📛 یوزرنیم: @{u['username'] or '---'}\n"
        f"💰 موجودی: <b>{fmt(u['wallet'])} تومان</b>\n"
        f"🛒 تعداد سفارش: {oc}\n"
...
def cb_ap_sub(call):
    ...
    bot.send_message(call.message.chat.id, f"➖ مبلغ کسر (تومان) از کیف پول <code>{uid}</code> را وارد کنید:")
...
def adm_modify_wallet(msg):
    ...
    if action == "adm_add_wallet":
        ...
        try:
            ...
            pass
        bot.send_message(msg.chat.id, f"✅ {fmt(amount)} تومان به {uid} اضافه شد. موجودی: {fmt(new_bal)}")
    else:
        dec = min(amount, get_wallet(uid))
        deduct_wallet(uid, dec)
        bot.send_message(msg.chat.id, f"➖ {fmt(dec)} تومان از {uid} کسر شد. موجودی: {fmt(get_wallet(uid))}")
    clear_state(ADMIN_ID)
